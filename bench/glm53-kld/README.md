...
scope          108 contexts scored of 112 captured (indices 0-114; 63/68/90/113 have no
                 reference shard - our reference-lane download is partial, see Retrieval),
...
**Why 108 of 112**: we captured 112 windows (indices 0-114), but four of them (63, 68, 90, 113)
have no reference shard on this host - the reference-lane download is partial (384 shards present,
index range 0-428, not contiguous) because the mirror died mid-fetch. Losing them is unbiased
(scattered across indices), and completing the fetch extends the row to 112 with a CPU re-score,
no cards. The FP8 lane is likewise partial (243 shards), which is why the anchor scored 48 contexts.
